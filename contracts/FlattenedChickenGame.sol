...
error FlattenedChickenGame__BetTooSmall();
error FlattenedChickenGame__BettingClosed();
error FlattenedChickenGame__UpkeepNotNeeded();
error FlattenedChickenGame__TransferFailed();
...
contract FlattenedChickenGame is Ownable {
    ...
    function bet() external payable {
        if (msg.value < i_betSize) {
            revert FlattenedChickenGame__BetTooSmall();
        }
        if (
            block.timestamp - s_lastBetTimestamp > i_interval && address(this).balance > msg.value
        ) {
            revert FlattenedChickenGame__BettingClosed();
        }
